...
- 更新 README，简化为 `send <实例> "<消息>"` 语法
- 添加关于自动配置 MCP 服务器的说明
- 移除过时的命令行示例
- 移除关于中文支持的限制说明（现已完全修复）
- 更新项目结构以反映 `mcp/send-tool/` 目录

**代码改进：**

- 添加 `CMW_VERBOSE` 环境变量控制调试输出
- 将硬编码的 DEBUG 输出替换为 `debug_print()` 函数
- 移除重复的异常处理代码
- 添加 `pyproject.toml` 用于正确的包管理
- 将 `send` 脚本移入 `mcp/send-tool/` 实现自包含 MCP 包

**新特性：**

- MCP 服务器现已完全自包含，可发布到官方 Registry
