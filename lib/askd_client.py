...
def try_daemon_request(
    ...
    # For Claude instances, use instance-specific session file
    session_filename = spec.session_filename
    if instance_id and instance_id != "default" and spec.protocol_prefix == "lask":
        from providers import get_claude_instance
        inst_spec = get_claude_instance(instance_id)
        session_filename = inst_spec.session_filename

    if not find_project_session_file(work_dir, session_filename):
        return None
...
def maybe_start_daemon(spec: ProviderClientSpec, work_dir: Path) -> bool:
    ...
    try:
        # Explicitly pass environment to ensure CCB_CLAUDE_INSTANCE is inherited
        ...
        if os.name == "nt":
            # Use CREATE_NEW_PROCESS_GROUP but NOT DETACHED_PROCESS
            # DETACHED_PROCESS can cause issues with network sockets
