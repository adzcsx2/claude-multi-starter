...
def _config_paths(work_dir: Path) -> Tuple[Path, Path]:
    # 优先使用根目录的配置文件，然后是 .cmw_config 目录
    project_root = Path(work_dir) / CONFIG_FILENAME
    project_cmw = Path(work_dir) / ".cmw_config" / CONFIG_FILENAME
    global_path = Path.home() / ".cmw" / CONFIG_FILENAME

    # 返回第一个存在的配置文件
    if project_root.exists():
        return project_root, global_path
    return project_cmw, global_path
