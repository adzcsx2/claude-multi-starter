...
from providers import LASKD_SPEC, get_claude_instance, ProviderDaemonSpec
...
class _SessionWorker(BaseSessionWorker[_QueuedTask, LaskdResult]):
    ...
    def _handle_task(self, task: _QueuedTask) -> LaskdResult:
        ...
        instance_id = req.instance_id or "default"
        _write_log(f"[INFO] start session={self.session_key} req_id={task.req_id} work_dir={req.work_dir} instance_id={instance_id}")

        session = load_project_session(work_dir, instance_id=instance_id)
        if not session:
            return LaskdResult(
                exit_code=1,
                reply=f"❌ No active Claude session found for instance '{instance_id}' in work_dir. Run 'ccb start {instance_id}' in that project first.",
                req_id=task.req_id,
                session_key=self.session_key,
                done_seen=False,
                done_ms=None,
...
class _WorkerPool:
    ...
    def submit(self, request: LaskdRequest) -> _QueuedTask:
        ...
        session = load_project_session(Path(request.work_dir), instance_id=request.instance_id)
        session_key = compute_session_key(session, instance_id=request.instance_id) if session else f"claude:{request.instance_id}:unknown"
...
class LaskdServer:
    def __init__(self, host: str = "127.0.0.1", port: int = 0, *, state_file: Optional[Path] = None):
        self.host = host
        self.port = port
        self.instance_id = os.environ.get("CCB_CLAUDE_INSTANCE", "default")
        self.instance_spec = get_claude_instance(self.instance_id)
        # Use instance-specific state file name if state_file not provided
        if state_file is None:
            self.state_file = state_file_path(self.instance_spec.state_file_name)
        else:
    ...
    def serve_forever(self) -> int:
        def _handle_request(msg: dict) -> dict:
            try:
                req = LaskdRequest(
                    ...
                    instance_id=str(msg.get("instance_id") or "") or self.instance_id,
                )
        ...
        # Create instance-specific daemon spec
        instance_daemon_spec = ProviderDaemonSpec(
            daemon_key=f"laskd_{self.instance_id}",
            protocol_prefix="lask",
            state_file_name=self.instance_spec.state_file_name,
            log_file_name=self.instance_spec.log_file_name,
            idle_timeout_env=self.instance_spec.idle_timeout_env,
            lock_name=self.instance_spec.lock_name,
        )

        server = AskDaemonServer(
            spec=instance_daemon_spec,
            host=self.host,
            port=self.port,
            token=self.token,
