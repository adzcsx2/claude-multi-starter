...
def load_project_session(work_dir: Path, instance_id: str = "default") -> Optional[ClaudeProjectSession]:
    """Load project session with instance_id support"""
    # Determine session filename based on instance_id
    session_filename = f".claude-{instance_id}-session" if instance_id != "default" else ".claude-session"

    resolution = resolve_claude_session(work_dir, session_filename=session_filename)
    if not resolution:
        return None
    ...
    if not session_file:
        try:
            ...
            session_file = project_config_dir(work_dir) / session_filename
        except Exception:
            session_file = None
    if not session_file:
        return None
...
def compute_session_key(session: ClaudeProjectSession, instance_id: str = "default") -> str:
    """Compute session key with instance_id support"""
    ...
    if instance_id and instance_id != "default":
        return f"claude:{instance_id}:{pid}" if pid else f"claude:{instance_id}:unknown"
